...
class DirFromPix(torch.nn.Module):
    def __init__(self, K, H, W, device='cuda'):
        super().__init__()
        self.cx = K[0][2]
        self.fx = K[0][0]
        self.cy = K[1][2]
        ...
        self.W=W
    def forward(self, xs):
        # convert pix to actual pix
        pixs = xs * torch.tensor([self.H-1, self.W-1], device=xs.device)
        # pixs[:, 0] = torch.clip(pixs[:, 0], min=0, max=self.shape[0]-1)
        # pixs[:, 1] = pixs[:, 1].clamp(min=0, max=self.shape[1]-1)
        return torch.stack([(pixs[:, 1]-self.cx)/self.fx,
                            -(pixs[:, 0]-self.cy)/self.fy,
                            -torch.ones(pixs.shape[0], device=self.device)], -1)
...
def train():
    ...
    heat_map = torch.ones((images.shape[0], H, W), dtype=torch.float, device=device) * 0.01
    ...
    next_batch = torch.zeros((images.shape[0], N_rand, 2), device=device)
    ...
    dfp = DirFromPix(K=K, H=H, W=W)
    args.image_sampling = True
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            if N_rand is not None:
                if i < args.precrop_iters:
                    select_coords = torch.rand([N_rand, 2], device=device) * 0.5 + 0.25
                    select_coords[:, 0] = select_coords[:, 0] 
                    ...
                    select_coords = torch.rand([N_rand, 2], device=device)
                    select_coords = select_coords * torch.tensor([H-1, W-1], device=device)
                    select_coords = select_coords.round() 
                    selected_coords_long = select_coords.long()
                    target_s = target[selected_coords_long[:, 0], selected_coords_long[:, 1]]  # (N_rand, 3)
                    ...
                    if next_batch[img_i].sum() == 0:
                        ...
                        select_coords = next_batch[img_i] * torch.tensor([H-1, W-1], device=device)
                        select_coords = select_coords.round() #+ 0.5
                        # val = pix_idxs[:, 0] * W + pix_idxs[:, 1]
                        selected_coords_long = select_coords.long()
                        target_s = target[selected_coords_long[:, 0], selected_coords_long[:, 1]]  # (N_rand, 3)
                        select_coords[:, 0] = select_coords[:, 0] / (H-1)
                        select_coords[:, 1] = select_coords[:, 1] / (W-1)
                ...
                # rays_o = rays_o[select_coords[:, 0].long(), select_coords[:, 1].long()]  # (N_rand, 3)
                # rays_d = rays_d[select_coords[:, 0].long(), select_coords[:, 1].long()]  # (N_rand, 3)
                # rays_d.requires_grad = True
        ...
        loss_per_pix = (((rgb - target_s) ** 2)).mean(-1)
        img_loss = torch.mean(loss_per_pix)
        imp_loss = torch.abs(rgb - target_s).mean(-1).detach()
        trans = extras['raw'][...,-1]
        loss = img_loss
        ...
        if i >= args.precrop_iters and args.sampling_type == "sgld-uni":
            ...
            if bound_idxs.shape[0] > 0:
                # print(bound_idxs.shape[0])
                if args.sampling_type == "sgld-uni":
                    next_batch[img_i][bound_idxs] = torch.rand([bound_idxs.shape[0], 2], device=device, dtype=torch.float32)
                # elif hparams.sampling_type == "sgld-mul":
                #     self.heat_map[batch['img_idxs']] = torch.clip(self.heat_map[batch['img_idxs']], min=1e-7)
                #     batch1d = torch.multinomial(self.heat_map[idx].flatten(), bound_idxs.shape[0], False)
                #     pix_idxs = torch.zeros((bound_idxs.shape[0], 2), device=device)
                #     pix_idxs[:, 1] =  (batch1d % self.heat_map[batch['img_idxs']].shape[1]) / float(self.train_dataset.img_wh[0]-1)
                #     pix_idxs[:, 0] = (batch1d / self.heat_map[batch['img_idxs']].shape[1]).long() / float(self.train_dataset.img_wh[1]-1)
                #     # pix_idxs[:, 0:1] = pix_idxs[:, 0:1] + ((torch.rand([bound_idxs.shape[0], 1], device=self.device)) / (float(self.train_dataset.img_wh[1]-1)))
                #     # pix_idxs[:, 1:2] = pix_idxs[:, 1:2] + ((torch.rand([bound_idxs.shape[0], 1], device=self.device)) / (float(self.train_dataset.img_wh[0]-1)))
                #     self.next_batch[idx][bound_idxs] = pix_idxs
            next_batch[img_i] = torch.clip(next_batch[img_i], min=0, max=1)
        ...
        if args.global_sampling:
            ...
            heat_map, heat_num, prob_map = update_heat_map_loss(imp_loss, img_i, selected_coords_long, heat_map, heat_num,
                prob_map)
